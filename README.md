...
为对应代理启用 `Use local HTTP bridge` 后，启动器只在回环地址上临时启动一个 HTTP CONNECT bridge（新配置默认是 `127.0.0.1:28083`），再将流量转发至该 SOCKS5 服务器。为兼容 ChatGPT 的内部组件，启动时会短暂设置 `launchctl` 代理变量；ChatGPT 启动后会立即清除，因此不会持续影响后续打开的其他应用。如果用户原本已有其他 `launchctl` 代理变量，启动器不会覆盖它们。bridge 会随本次启动的 ChatGPT 主进程结束而停止；如果启动器意外退出，bridge 也会自行回收。配置端口已被占用时会明确报错，不会静默递增并产生多个监听。启动器不会改动系统 HTTP 代理。
...
For that proxy profile, enable **Use local HTTP bridge**. The launcher temporarily opens an HTTP CONNECT bridge on a loopback address (`127.0.0.1:28083` by default for new configurations) and forwards it to the selected SOCKS5 server. To support ChatGPT internal components, it briefly sets proxy variables through `launchctl` during startup, then clears them immediately so later applications are not continuously affected. Existing unrelated `launchctl` proxy variables are not overwritten. The bridge stops when the ChatGPT main process started by this launcher exits, and also self-cleans if the launcher exits unexpectedly. If the configured port is already occupied, startup fails with an explicit error instead of silently opening additional ports. It never changes the system HTTP proxy.
